...
class Settings(BaseSettings):
    ...
    VMR_TARGET_MODE: str = Field(default="NONE",description="Target mode: NONE (trail-only), VWAP, or FIXED_RR")
    ...
    VMR_MAX_POSITION_VALUE_PCT: float = Field(default=30.0, description="Max position value as % of total capital")
    VMR_DAILY_LEVEL_FILTER: bool = Field(default=True, description="Require pin bar near daily S/R level")
    VMR_DAILY_LEVEL_BUFFER_PCT: float = Field(default=0.5, description="Buffer % for daily level proximity check")
    VMR_TRAIL_ACTIVATE_R: float = Field(default=1.0,description="Activate trailing stop after price moves 1x risk in your favor")
    VMR_TRAIL_METHOD: str = Field(default="SWING",description="Trail method: SWING (previous candle low/high) or PERCENT (lock % of MFE)")
    VMR_TRAIL_SWING_LOOKBACK: int = Field(default=3,description="Number of candles to look back for swing trail level")
    VMR_TRAIL_LOCK_BREAKEVEN: bool = Field(default=True,description="Once trail activates, move SL to breakeven as a floor")
